...
const ProductEdit = () => {
  ...
  const {
    rootStore: {
      productStore: { fetchCategories, getProductData, updateData },
    },
  } = useStore();
  ...
  useEffect(() => {
    const initForm = async () => {
      try {
        if (id) {
          const resData = await getProductData(id);
          if (resData.data !== undefined) {
            // console.log(resData);
            let { image, ...formData } = resData?.data?.product;
            setImageUrl(`${process.env.REACT_APP_API_STORAGE_URL}/${image}`);
            reset(formData);
          }
        } else {
          navigate(-1);
  ...
  }, [id, getProductData, navigate, reset]);
